    ...
    for (const item of files) {
      ...
      html += `
        <li class="flex items-center justify-between p-1.5 rounded hover:bg-gray-800/60 cursor-pointer group"
            onclick="window.quickRunCommand('cat ${item.path}')">
          <span class="flex items-center gap-1.5 truncate">
            <span>${icon}</span>
            <span class="${isDir ? 'text-teal-300 font-semibold' : 'text-gray-300'} group-hover:text-cyan-400">${item.path}</span>
          </span>
          ${sizeStr}
        </li>
      `;
      ...
      html += `
        <div class="border border-gray-800 bg-gray-900/60 rounded p-3 hover:border-teal-500/50 transition">
          <div class="flex items-center justify-between mb-1">
            <span class="font-bold text-teal-400">${tool.name}</span>
            <span class="text-[10px] px-1.5 py-0.5 rounded bg-gray-800 text-gray-400">tool</span>
          </div>
          <p class="text-gray-400 text-xs mb-2 leading-relaxed">${tool.description}</p>
          <pre class="bg-black/50 p-2 rounded text-[11px] text-gray-300 overflow-x-auto">${JSON.stringify(tool.inputSchema, null, 2)}</pre>
        </div>
      `;
    }
    html += '</div>';
      ...
      html += `
        <div class="p-2.5 rounded border border-gray-800 bg-gray-950/50 space-y-1">
          <div class="flex items-center justify-between">
            <span class="font-semibold text-cyan-400">${log.tool}</span>
            <span class="text-[10px] px-1.5 py-0.5 rounded border ${statusColor}">${log.status} (${log.durationMs}ms)</span>
          </div>
          <div class="text-[11px] text-gray-400 truncate">Args: ${JSON.stringify(log.args)}</div>
          ${log.result ? `<div class="text-[11px] text-gray-300 truncate">Result: ${JSON.stringify(log.result)}</div>` : ''}
          ${log.error ? `<div class="text-[11px] text-red-400 truncate">Error: ${log.error}</div>` : ''}
        </div>
      `;
    }
    html += '</div>';
